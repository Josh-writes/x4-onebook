...
int InputManager::getButtonFromADC(const int adcValue, const int ranges[], const int numButtons) {
  for (int i = 0; i < numButtons; i++) {
    if (ranges[i + 1] < adcValue && adcValue <= ranges[i]) return i;
  }
  return -1;
}
...
void InputManager::update() {
  ...
  pressedEvents = 0;
  releasedEvents = 0;
  if (state != lastState) { lastDebounceTime = currentTime; lastState = state; }
  if ((currentTime - lastDebounceTime) > DEBOUNCE_DELAY) {
    if (state != currentState) {
      pressedEvents  = state & ~currentState;
      releasedEvents = currentState & ~state;
      if (pressedEvents > 0 && currentState == 0) buttonPressStart = currentTime;
      if (releasedEvents > 0 && state == 0)        buttonPressFinish = currentTime;
      currentState = state;
    }
  }
}
...
const char* InputManager::getButtonName(const uint8_t buttonIndex) {
  if (buttonIndex <= BTN_POWER) return BUTTON_NAMES[buttonIndex];
  return "Unknown";
}
